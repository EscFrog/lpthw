class 노래(object):
    ...
    def __init__(self, 가사): # 초기화 할 때 가사 인수에 인자를 받는다. 밑에서 for 구문을 사용해야 하므로 iterable 형태로 받아야 한다.
        self.가사 = 가사
    
    def 노래_불러(self): # 함수를 실행할 때, 현재 클래스로 생성된 수많은 객체 중 호출된 자기 자신임을 명시적으로 알려줘야 한다.
        for 한줄 in self.가사:
            print(한줄)
